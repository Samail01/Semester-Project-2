...
import { fetchProfile, updateProfileAvatar, createListing } from "./api.js";
...
async function displayProfileDetails(name) {
  ...
  if (profile) {
    ...
    createListingForm.addEventListener("submit", async (event) => {
      ...
      const media = [{ url: mediaUrl, alt: "Image of " + title }];
      ...
      try {
        const result = await createListing(title, description, media, endsAt);
        if (result) {
    ...
    if (profile.listings && profile.listings.length > 0) {
      profile.listings.forEach((listing) => {
        ...
        listingItem.innerText = listing.title;
